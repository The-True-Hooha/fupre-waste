...
const HeroSection = () => {
  ...
  return (
    <div className="md:items-center flex flex-col ">
      <div
        className=" font-medium 2xl:w-1/3 md:w-2/3 xl:w-1/2 lg:px-0 px-8 text-[20px] xl:text-6xl flex justify-center xl:font-medium xl:pt-14 text-center pt-6"
      >
        Fupre Centre for Waste Management and Sustainable Resources
      </div>

      <p
        className="text-[10px] pt-4 text-center w-2/3 mx-auto"
      >
        The FUPRE Centre for Waste Management and Sustainable Resources is dedicated to innovating environmentally-friendly solutions for waste management and resource sustainability.
      </p>
      ...
      {isSmallScreen ? (
        <div className="px-8">
          <div className="grid grid-cols-3   md:row-span-1  gap-3  xl:gap-6 mt-8   xl:px-0  ">
            {tabs.map((tab) => (
              <motion.div
                key={tab.name}
                className={`
        ...
        <div className="flex  xl:space-x-3 items-center justify-between hover:cursor-pointer gap-3 w-4/5 xl:w-3/4 2xl:w-[55%]">
          {tabs.map((tab) => (
            <motion.div
              key={tab.name}
              className={`
